...
/**
 * {@link SwitchCompat}：在 CompoundButton AppCompat 别名之上跟踪 thumb / track 及其 tint。
 * 写入走 {@link SwitchCompat} 自身 API（各 API 级可用），不经平台 {@code Switch}。
 */
public class SwitchCompatResourceBinding extends AppCompatCompoundButtonResourceBinding {

    /** {@code app:track}，和 {@link SwitchResourceBinding#ATTR_TRACK} 不是同一个 id。 */
